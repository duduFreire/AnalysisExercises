...
\newenvironment{shortlemma}[1]{\refstepcounter{lemmaCounter}\label{#1}\textbf{Lemma~\thelemmaCounter.}}

\newenvironment{shorttheorem}[1]{\refstepcounter{theoremCounter}\label{#1}\textbf{Theorem~\thetheoremCounter.}}
...
\end{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{gather*}
            ...
            x_n := \inf{A_n} \\
            y_n := \sup{A_n}.
        \end{gather*} Since $\sup{B} \geq \inf{B}$ for any non-empty bounded set $B$, we have $y_n \geq x_n$ for every natural $n$, so we can apply the Order Limit Theorem to get $\lim y_n \geq \lim x_n$,
