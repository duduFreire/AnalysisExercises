...
\newenvironment{shorttheorem}[1]{\refstepcounter{theoremCounter}
\label{#1}\textbf{Theorem~\thetheoremCounter.}\em}

\newenvironment{shortdefinition}[1]{\refstepcounter{definitionCounter}
\label{#1}\textbf{Definition~\thedefinitionCounter.}\em}
...
\end{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            \lim_{n \ra \infty} \left(\frac{2n+k}{n(n+k)}\right) = 0,
    ...
    \item 
    \begin{shortdefinition}{def_equivSequences}
    ...
    \begin{shortlemma}{lem_equivCauchySequences}
        If $(a_n)$ is equivalent to $(b_n)$ and $(b_n) \ra b$, it follows that $(a_n) \ra b$.
    \end{shortlemma}
    \begin{proof}
    ...
    \begin{enumerate}
        ...
        \item This is almost identical to the previous proof. Construct the intervals $I_n$ just as before. By the Nested Interval Property, we can choose some $x \in \R$ such that $x \in I_n$ for every $n$. To prove that $(S_n) \ra x$, \lep[l]. Choose $N \in \N$ such that $\abs*{a_{2n}} < \epsilon$ for all $n \geq N$ and define $M := 2N-1$. Then, since $S_n,x \in I_N$ for all $n \geq M$, and the length of $I_N$ is less than $\epsilon$, we must have $\abs*{S_n-x} < \epsilon$ for all $n \geq M$.
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            \sum_{k=1}^\infty s_k(y_k-y_{k+1})
        \end{equation*} converges, we really only needed $(s_n)$ to be bounded, so the same proof applies. The other slight difference is when we applied the Algebraic Limit Theorem to show that $s_n y_{n+1}$ converges. This does not work here, since $(s_n)$ does not necessarily converge. To see that this is not a problem, \lep. Let $M \in \R$ be a bound for $(s_n)$. Use the fact that $(y_n) \ra 0$ to choose $N \in \N$ such that $\abs*{y_n} < \epsilon/M$ for all $n \geq M$. Then, $\abs*{s_n y_{n+1}} \leq M \abs*{y_{n+1}} < \epsilon$, therefore $(s_n y_{n+1}) \ra 0$. For the same reason as before, this means that
        \begin{equation*}
            \sum_{k=1}^\infty x_k y_k
        \end{equation*} converges.
        
        \item Let $(x_k) = (-1)^{k+1}$ and $(y_n) \ra 0$ be decreasing. Notice that the partial sums of $(x_k)$ are bounded by $1$. By Dirichlet's Test, 
        \begin{equation*}
    ...
    \begin{enumerate}
        ...
        \item Let $O_n := (-1/n, 1/n)$ for every natural $n$, and define $O := \bigcap_{n=1}^\infty O_n$. It is easy to verify that $0 \in O$. Now, let $x \in O$.. If $x > 0$, we can pick a natural $N$ such that $N > 1/x$. Then, $x > 1/N$, therefore $x \notin O_N$ which contradicts our assumption that $x \in O$. A similar contradiction arises when $x < 0$, which means $x$ can only be zero, so we have $O = \set{0}$. The only sequence that is contained in $A$ is $(0, 0, 0, \dots) \ra 0$, so $O$ has no limit points, therefore it is closed.
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Not compact. We have already shown in Exercise 3.2.3 (d) that the set $\set{\sum_{k=1}^1 1/k^2 : n \in \N}$ does not contain its supremum, therefore it cannot be compact, by Exercise 3.3.1. The sequence $(a_n) = (1/n^2)$ is contained in $K$ and converges to $\sup K$.
    ...
    \begin{enumerate}
        ...
        \setcounter{enumiii}{2}
            ...
            \end{align*} whenever $0 < \abs*{x-c} < \delta$, therefore $\lim_{x \ra c} [f(x)g(x)] = L M$.
            
            \item It is sufficient to show that $\lim_{x \ra c} 1/g(x) = 1/M$. \lep. Choose $\delta_1 > 0$ such that $\abs*{g(x)-M} < \abs*{M}/2$ for all $0 < \abs*{x-c}<\delta_1$. It follows that $\abs*{g(x)} > \abs*{M}/2 > 0$ for the appropriate $x$. Now, choose $\delta_2 > 0$ such that $\abs*{g(x) - M} < \epsilon \abs*{M}^2/2$ for $x$ in the expected interval. Setting $\delta := \min (\delta_1, \delta_2)$, it follows that 
            \begin{equation*}
